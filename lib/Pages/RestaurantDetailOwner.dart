...
class RestaurantDetailData {
  ...
  factory RestaurantDetailData.fromJson(Map<String, dynamic> json) {
    // Handle both local SQLite format and HTTP format
    final pictures = (json['pictures'] is List)
        ? List<String>.from(json['pictures'])
        : <String>[];

    final workTime = (json['work_time'] is List)
        ? (json['work_time'] as List).map((item) => WorkTime.fromJson(item)).toList()
        : <WorkTime>[];
...
class _RestaurantDetailOwnerState extends State<RestaurantDetailOwner> {
  ...
  Future<void> _loadRestaurantDetails() async {
    try {
      // Use local RestaurantService instead of HTTP
      final restaurantData = await RestaurantService.instance.getRestaurant(widget.restaurantId);

      if (mounted) {
        if (restaurantData != null) {
          setState(() {
            _restaurant = RestaurantDetailData.fromJson(restaurantData);
            _isLoading = false;
          });
        } else {
          setState(() {
          ...
          children: [
            PageView.builder(
              ...
              itemBuilder: (context, index) {
                final pictureData = _restaurant!.pictures[index];
                ...
                return Container(
                  ...
                  child: _isBase64Image(pictureData)
                      ? Image.memory(
                    _base64ToImage(pictureData),
                    fit: BoxFit.cover,
  ...
  Uint8List _base64ToImage(String base64String) {
    try {
      ...
      return Uint8List.fromList([137, 80, 78, 71, 13, 10, 26, 10, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 1, 0, 0, 0, 1, 8, 6, 0, 0, 0, 31, 21, 196, 137, 0, 0, 0, 13, 73, 68, 65, 84, 120, 156, 99, 248, 15, 0, 0, 1, 1, 0, 5, 0, 0, 0, 0, 13, 10, 45, 180, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66, 96, 130]);
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: () {
            // FIXED: Simple safe back navigation
